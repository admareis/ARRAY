...
console.log(produtos[9].preco_venda);

console.log(Array.isArray(frutas));

console.table(produtos);

console.log(produtos[24].estoque);

* para adicionar um item no final da lista *

produtos.push({id: 101, nome: "Água", preco_venda: 2.30, preco_compra: 1.20, estoque: 80, rotatividade: "media", curva_abc: "B"});
...
* para excluir somente uma linha * 

produtos.splice(13,1);

console.table(produtos);

* para puxar um nome do objeto em especifico * 

let nomes = produtos.map((produto) => produto.nome)

//console.log(nomes)

* para criar uma nova array (COLUNA) * 

let desconto = produtos.map((produto) => ({...produto, desconto: produto.preco_venda * 0.9}))

console.table(desconto)

* para puxar itens especificos * 

let estoque_Baixo = produtos.filter(produto => produto.estoque <=50)
console.table(estoque_Baixo)

let pedido_Compra= produtos.filter(produto => produto.estoque <=50 && produto.rotatividade== "alta" && produto.curva_abc== "A");
console.table(pedido_Compra)
